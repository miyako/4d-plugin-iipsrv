...
class Cache {
 ...
 public:
  ...
  unsigned int getNumElements() { return tileList.size(); }
	...
	unsigned long getMaxSize() { return maxSize / 1024000.0 ; }
  ...
  RawTile* getTile( std::string f, int r, int t, int h, int v, CompressionType c, int q ) {
	...
	std::string getRegionIndex(std::string f, int r, int seq, int ang, int layers, unsigned int x, unsigned int y, unsigned int width, unsigned int height)
	{
		char tmp[1024];
		snprintf( tmp, 1024, "%s:%d:%d:%d:%d:%d:%d:%d:%d", f.c_str(), r, seq, ang, layers, x, y, width, height );
