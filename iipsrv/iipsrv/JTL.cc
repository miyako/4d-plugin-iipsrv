...
void JTL::doSend( Session* session, int resolution, int tile )
{	
  ...
  if( (*session->image)->getNumBitsPerPixel() > 8 || (*session->image)->getColourSpace() == CIELAB
	...
	session->key = "JTL:" + tilemanager.getIndex(resolution,
																							 tile,
																							 session->view->xangle,
																							 session->view->yangle,
  ...
  if( rawtile.compressionType == UNCOMPRESSED ){
		...
		outData_putStr(session->out, static_cast<const char*>(rawtile.data), len);
...
{	
	...
	if((format == JPEG2000) && (!session->key_only))
	{
		std::lock_guard<std::mutex> guard(*session->openJPEGMutex);
		this->doSend(session, resolution, tile);
	}else
	{
		this->doSend(session, resolution, tile);
	}
