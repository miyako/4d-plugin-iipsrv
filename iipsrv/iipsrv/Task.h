...
void outData_printf(std::vector<uint8_t> *writer, const char *str);
void outData_putStr(std::vector<uint8_t> *writer, const char *str, size_t len);
...
#define IIP_EVENT_FIND_IMAGE 0
#define IIP_EVENT_SEND_IMAGE 1
...
class JTL : public Task {
 public:
  ...
  void send( Session* session, int resolution, int tile );
	void doSend( Session* session, int resolution, int tile );
};
