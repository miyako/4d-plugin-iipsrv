...
class TileManager{
 ...
 public:
  ...
  TileManager( Cache* tc, IIPImage* im, Watermark* w, Compressor* j, std::ofstream* s, int l ){
	...
	std::string getRegionIndex( unsigned int res, int seq, int ang, int layers, unsigned int x, unsigned int y, unsigned int width, unsigned int height );
  ...
  RawTile getTile( int resolution, int tile, int xangle, int yangle, int layers, CompressionType c );
	...
	std::string getIndex( int resolution, int tile, int xangle, int yangle, int layers, CompressionType c );
